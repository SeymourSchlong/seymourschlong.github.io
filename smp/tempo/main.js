...
const submit = () => {
    ...
    let tempo = document.querySelector('#bpm').value * select.options[select.selectedIndex].value;
...
const init = () => {
    ...
    bpmInput.addEventListener('keydown', evt => {
        if (evt.keyCode == 13) {
            evt.preventDefault();
            submit();
        }
    ...
    document.addEventListener('keydown', evt => {
        if (evt.key === 'j') {
            ...
            if (count == 0) {
                ...
                bpmInput.value = Math.floor(Math.round(60000 * count / (msecs - msecsFirst) * 10) / 10);
                submit();
                count++;
            }
            msecsPrevious = msecs;
